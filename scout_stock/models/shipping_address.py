...
class SaleOrderShipping(models.Model):
	...
	@api.multi
	def culate_nso_lines_order(self,order):
		for order in self:
	...
	@api.multi
	def button_culate_nso_lines_order(self,order):						
						...
						if order.partner_shipping_id.country_id.code == 'PH':
							...
							if delivery_carrier:
								...
								if not res_price.get('error_message'):
									...
									order.culate_nso_lines_order(order)
							...
							if delivery_carrier:
								...
								if not res_price.get('error_message'):
									...
									if currency:
										if order.currency_id != order.company_id.currency_id:
											payment_processing_fee = currency._convert(payment_processing_fee,order.currency_id.order._compute_nso_amount_delivery_id,fields.Date.today())
									handling_price = (pre_price.get('price') *handling_price)/100
									temp_price = payment_processing_fee + ((transaction_value/100) * (res_price.get('price') + handling_price))
									line.write({
										'delivery_method':delivery_carrier.id,
										'delivery_charge':res_price.get('price') + temp_price
										})
